module ApplicationHelper
  def active_nav_class(resource)
    match = case resource
            when "pricing" then action_name == "pricing"
            else controller_name == resource
            end
    match ? "text-white" : ""
  end
  ...
  def status_badge_class(status)
    case status.to_s
    when "running", "success"            then "bg-emerald-500/10 text-emerald-400 ring-emerald-500/20"
    when "failed"                        then "bg-red-500/10 text-red-400 ring-red-500/20"
    when "building", "deploying", "health_check" then "bg-yellow-500/10 text-yellow-400 ring-yellow-500/20"
  ...
  def status_spinning?(status)
    %w[queued pending analyzing cloning detecting building deploying health_check].include?(status.to_s)
  end
